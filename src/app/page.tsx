            ...
            <div className="mt-12 grid gap-6 lg:grid-cols-3">
              {featured.map((exhibition, index) => (
                  <Link key={exhibition.id} href={`/exhibitions/${exhibition.id}`} className={`group overflow-hidden rounded-sm border border-[#0a2348]/12 bg-white shadow-[0_12px_40px_rgba(10,35,72,.06)] transition duration-300 hover:-translate-y-1 hover:border-[#0867d9]/45 hover:shadow-[0_20px_50px_rgba(8,103,217,.12)] ${index === 0 ? "lg:col-span-2" : ""}`}>
                    ...
                    <div className="flex min-h-64 flex-col justify-between p-6 sm:p-7"><div><p className="eyebrow text-[#0867d9]">{new Date(exhibition.startDate).toLocaleDateString("en-IN", { month: "short", day: "numeric" })} - {new Date(exhibition.endDate).toLocaleDateString("en-IN", { month: "short", day: "numeric", year: "numeric" })}</p><h3 className="mt-4 text-2xl font-bold text-[#0a2348]">{exhibition.name}</h3><p className="mt-3 line-clamp-2 text-sm leading-6 text-[#0a2348]/65">{exhibition.description}</p></div><p className="mt-8 text-sm font-bold text-[#0a2348]/80">{exhibition.venue} <span className="float-right text-[#0867d9] transition group-hover:translate-x-1">Explore ↗</span></p></div>
                </Link>
              ))}
            </div>
